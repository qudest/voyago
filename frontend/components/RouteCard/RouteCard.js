...
const RouteCard = ({
  ...
  const [likeRoute, setLikeRoute] = useState(isLiked);
  const contentRef = useRef(null);
  const navigation = useNavigation();

  const { title = "", time = "", distance = "", points = [] } = cardInformation;
    ...
    <TouchableOpacity activeOpacity={0.9} onPress={handleToggleExpand}>
      <Animated.View style={[styles.container, { height: cardHeight }]}>
        <View style={styles.header}>
          ...
          <Image
            source={
              !expanded && require("../../assets/routeCardImages/bottom.png")
            }
