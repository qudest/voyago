...
import { View, Image, ActivityIndicator, Text, ScrollView, SafeAreaView, Alert } from 'react-native';
...
const PreviewRouteScreen = ({ navigation, route }) => {
    ...
    const fetchRouteData = async (routeParams) => {
        try {
            ...
            const destinationCoords = await getCoordinatesFromPlaceId(routePoints.destination);

            const waypointsParam = routePoints.waypoints.length > 0 
                ? `optimize:true|${routePoints.waypoints.map(wp => {
            ...
            const response = await axios.get(
                ...
                {
                    params: {
                        origin: `place_id:${String(routePoints.origin).replace('place_id:', '')}`,
                        destination: `place_id:${String(routePoints.destination).replace('place_id:', '')}`,
            ...
            if (response.data.routes.length > 0) {
                ...
                const region = {
                    ...
                    latitudeDelta: Math.abs(Math.max(...latitudes) - Math.min(...latitudes)) * 1.5 + 0.01,
                    longitudeDelta: Math.abs(Math.max(...longitudes) - Math.min(...longitudes)) * 1.5 + 0.01
                };
                ...
                return {
                    ...
                    markers,
                    region,
                    distance: distance || route.legs.reduce((sum, leg) => sum + leg.distance.value, 0),
                    duration: duration || route.legs.reduce((sum, leg) => sum + leg.duration.value, 0),
                    points: pointNames
                };
            }
